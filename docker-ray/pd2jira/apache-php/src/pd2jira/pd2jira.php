...
$jira_url = getenv('PAGER2J_JIRA_URL');
if (substr($jira_url, strlen($jira_url)-1, 1) == "/") {
  $jira_url = substr($jira_url, 0, strlen($jira_url)-1);
}
$jira_username = getenv('PAGER2J_JIRA_USERNAME');
$jira_password = getenv('PAGER2J_JIRA_PASSWORD');
$jira_project = getenv('PAGER2J_JIRA_PROJECT');
$jira_issue_type = getenv('PAGER2J_JIRA_ISSUE_TYPE');
$pd_subdomain = getenv('PAGER2J_PAGERDUTY_SUBDOMAIN');
$pd_api_token = getenv('PAGER2J_PAGERDUTY_API_TOKEN');
//$ecosystem = getenv('PAGER2J_ECOSYSTEM_LOCATION');
...
if ($messages) foreach ($messages->messages as $webhook) {
  ...
  switch ($webhook_type) {
    case "incident.trigger":
      ...
      $data = array('fields'=>array('project'=>array('key'=>"$jira_project"), 'customfield_13603'=>array('value'=>"LV - Prod")  ,'summary'=>"$summary",'description'=>"A new PagerDuty ticket has been created.  {$trigger_summary_data}. Please go to $ticket_url to view it.", 'issuetype'=>array('name'=>"$jira_issue_type")));
